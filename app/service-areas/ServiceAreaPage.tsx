...
export default function ServiceAreaPage({ area }: { area: ServiceArea }) {
  ...
  const faqSchema = { "@type": "FAQPage", mainEntity: area.faq.map(([name, text]) => ({ "@type": "Question", name, acceptedAnswer: { "@type": "Answer", text } })) };
  const schema = {
  ...
  return (
    <main className="service-page">
      ...
      <section className="trust-strip" aria-label="What to expect"><div><strong>Interior painting</strong><span>Walls, ceilings, trim, doors, and living spaces.</span></div><div><strong>Exterior painting</strong><span>Arizona-aware prep and coatings for stucco and exterior surfaces.</span></div><div><strong>Cabinet refinishing</strong><span>Detailed preparation and durable kitchen finishes.</span></div><div><strong>Garage floor coatings</strong><span>Concrete preparation and coating systems built around the slab.</span></div></section>

      <section className="service-benefits section"><div><p className="eyebrow">Serving {area.city}</p><h2>Local service.<br /><em>A higher standard.</em></h2><p>{area.localContext}</p><p><strong>{area.exteriorNote}</strong></p></div><div className="benefit-grid"><article><span>01</span><h3>Arizona-ready preparation</h3><p>We inspect surface condition and tailor preparation to sun exposure, stucco, trim, previous coatings, and the needs of your property.</p></article><article><span>02</span><h3>Clear project scope</h3><p>You receive a documented scope so included surfaces, preparation, and finish expectations are clear before work begins.</p></article><article><span>03</span><h3>Professional finish</h3><p>Clean lines, careful protection, professional coatings, and a final walkthrough are built into our process.</p></article></div></section>
      ...
      <section className="service-faq section"><div><p className="eyebrow">Local questions</p><h2>Painting in {area.city},<br /><em>answered clearly.</em></h2></div><div>{area.faq.map(([question, answer], index) => <details key={question} open={index === 0}><summary>{question}<span>+</span></summary><p>{answer}</p></details>)}</div></section>
